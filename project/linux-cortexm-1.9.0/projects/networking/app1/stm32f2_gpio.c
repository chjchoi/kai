...
// shortLoopBack
struct stm32f2_gpio_dsc short_loopback={6,7};
//tp_sync
 ...
 */
static const unsigned int af_val[] = {
...
int stm32f2_gpio_config(struct stm32f2_gpio_dsc *dsc,enum stm32f2_gpio_role role)
{
	...
	unsigned int	otype, ospeed=0, pupd, i,mode;//NOR
	int	rv;
...
int stm32f2_led_init(void)
{
	...
	{
		printf( "led1 init error:open:%d\n",rv);
	}
	rv=-1;
	if((rv=stm32f2_gpio_config(&led2,STM32F2_GPIO_ROLE_GPOUT))<0);
	{
		if(rv!=0)
		printf( "led2 init error:open:%d\n",rv);
	}		
}
