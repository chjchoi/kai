...
int main(void)
{
	...
	{
		case -1: //fork fail
			close(fd1);close(fd3);close(fd5);
			return -1;
		case 0://child
			execl("/mnt/ttyX","ttyX","1",0);
			break;
		default:
			pid3=vfork();
			switch(pid3)
...
int nor_Show_Diag_Menu()
{
	...
	printf("n) usart(n) test code send,  1) usart1 test code send \n");
	printf("z) quit\n");
	return 0;
} 
//
