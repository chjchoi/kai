...
static s32 stm_phy_link_setup(struct stm_eth_dev *mac)
{
	...
	while (timeout-- > 0) {
		...
		/*		
		rv =stm_phy_read(mac, PHY_BMSR, &val);
		if(rv==0 && (val & PHY_BMSR_AUTN_COMP))
		{
			timeout =0;
		}
		*/
		else
		{
			...
			if(rv!=0)
			{
				printf(" stm_phy_read fail. 2..\n");
			}
			else
			{
				printf("phy read ==0 val & PHY_BMSR_AUTN_COMP ==0:val is %d\n",val);
			}
			*/
			udelay(100);
		}
...
static s32 stm_phy_read(struct stm_eth_dev *mac, u16 reg, u16 *val)
{
	...
	if (stm_phy_wait_busy(STM32_PHY_READ_TIMEOUT) != 0) {
		 ...
		 */
		printf(" stm32_phy_wait_busy.. read timeout\n");
		rv = -ETIMEDOUT;
		goto out;
	}
...
static s32 stm_mac_hw_init(struct stm_eth_dev *mac)
{
	...
	if (stm_phy_wait_busy(STM32_PHY_READ_TIMEOUT) != 0) {
		 ...
		 * MII is busy
		 */
		printf(" stm_phy_wait_busy timeout\n");//NOR
		rv = -EBUSY;
		goto out;
	}
	STM32_MAC->macmiiar = tmp;
	...
	if (rv != 0)
	{
		printf("stm_phy_link_setup fail\n"); //NOR
		goto out;
	}
out:
	return rv;
